...
- 高さ: 60px（sticky、`z-index: 100`）
- ロゴ: `height: 44px`（画像は1000×300px）
- PC（701px以上）: ロゴ左 ＋ テキストナビ右（4項目）
- スマホ（700px以下）: ロゴ左 ＋ ハンバーガーボタン右
  - ハンバーガーは JS で `.is-open` クラスをトグル
...
- 全ページが `../style.css` を参照
- ヘッダーは index.html と同一構成（ただし nav はなし、ハンバーガーなし）
- `.legal-wrap` クラスで統一レイアウト（max-width: 760px、margin: 48px auto）
...
- サブページのヘッダーにはナビ・ハンバーガーを追加していない（法的ページはシンプルで良い）
